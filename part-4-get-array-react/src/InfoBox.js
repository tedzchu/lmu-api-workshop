  ...
  componentDidMount(){
    this.getData = () => {
      ...
      fetch(url).then(r => r.json())
        .then((stockData) => {
          const price = stockData['Time Series (Daily)'][Object.keys(stockData['Time Series (Daily)'])[0]]['4. close'];
          const change = price - data[0].y;
          const changeP = (price - data[0].y) / data[0].y * 100;

          this.setState({
            currentPrice: stockData['Time Series (Daily)'][Object.keys(stockData['Time Series (Daily)'])[0]]['4. close'],
