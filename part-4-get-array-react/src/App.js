      ...
      fetch(url).then( r => r.json())
        .then((stockData) => {
          ...
          for (let date in stockData['Time Series (Daily)']){
            console.log(date, count, stockData['Time Series (Daily)'][date]['4. close'])
            sortedData.unshift({
              d: moment(date).format('MMM DD'),
              p: stockData['Time Series (Daily)'][date]['4. close'].toLocaleString('us-EN',{ style: 'currency', currency: 'USD' }),
